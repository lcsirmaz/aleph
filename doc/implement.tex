...
requires a single extra bit only, which specifies whether the item is a
pointer or not. When opening an \A{} datafile, stored lists are paired
with the specified ones so that the appropriate pointer transformation could
be made. How the pairing should be done is not specified by the \A{} Manual. One
possibility is to match them by position; another one is to match them by name.
This implementation uses the second possibility, but instead of the list
name it uses a 32-bit hash value of the name stored in the file. When
...
exception. Making modular programming possible -- as opposed to using monolith
programs prevalent at the time of the creation of \A{} -- required some new
features. Other extensions and restrictions came up naturally; some of them
date back to the time of the first \A{} compilers in the early 80's. Changes
introduced here respect the original design ideas and philosophy as much
as possible.
...
are not characters. This implementation relaxes this restriction, and the
...
implementation restricts them to \g{constant}s and \g{variable declaration}s
