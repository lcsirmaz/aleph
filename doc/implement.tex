...
In a \g{datafile} declaration, all lists must be specified to which the
datafile can have pointers. By storing the list names in the datafile first,
each additional item requires a single extra bit only, which specifies
whether the item is a pointer or not. When opening an \A{} datafile, stored
list names are paired with the names of the specified ones so that the
appropriate pointer[B transformation could be made. How the pairing should
be done is not specified by the \A{} Manual. One possibility is to match
them by position; another one is to match them by name. This implementation
uses the second possibility, but instead of the list name it uses a 32-bit
hash value of the name stored in the file. When opening a datafile, each
specified list is matched to the one with the same hash. It is an error if a
pointer in the file has no matching list.

\section{Syntax}

The \A{} programming language is defined in the \emph{Manual} which was
...
features, restrict or leave out others. This happened with \A{} as well;
...
exception. Making modular programming possible -- as opposed to using
monolith programs prevalent at the time of the creation of \A{} -- required
some new features. Other extensions and restrictions came up naturally; some
of them date back to the time of the first \A{} compilers in the early 80's.
The changes respect the original design ideas and philosophy as much as
possible.
...
a single (monospace) typeface. There are several approaches to
distinguishing
...
\end{program}
\noindent
Other approaches are using \pp|CAPITAL| letters for keywords (as
...
whitespace at the end (i.e., leave out the closing apostrophe); etc.
...
are not characters. This implementation relaxes this restriction, and
...
the same line they start in. Two separate consecutive strings (even if they
are on different lines) are concatenated, thus strings can be continued on
the next line.

\subsection{Hexadecimal constants}

Whereever integers are accepted, constants in hexadecimal notation are
...
Similar shorthands were added (that were not present in the original language)
...
\end{program}
\noindent
This extension means that occasionally a \g{stack} looks -- and
behaves -- like a variable, making writing and comprehending stack operations
easier.

This extension comes naturally, and is really handy in cases when the
content of a newly added block on the top of a \g{stack} has to be
manipulated. In cases when the list, as an affix, was added by mistake, the extension
prevents the compiler from complaining about an affix type mismatch. The
implicit reference to the topmost list block comes, unexpectedly, with an extra
complication during macro substitution, see Section \ref{sec:macro}.
...
(i.e. stacks without size estimates) have equal actual and virtual limits.
...
the \g{dummy symbol}
can be used. This symbol has representation \pp|?| in the Manual, but our
implementation also accepts the sharp-sign \pp|\#|. We consider the
latter more appropriate as it does not indicate
any uncertainty. Compare the two lines below:
\begin{program}
 divrem+a+5+\#+rem,  
 divrem+x+10+?+q.
\end{program}
...
simplifies writing some program texts, and, hopefully, makes them more
transparent by bringing the string definition closer to its usage.
Without this feature strings are typically put into a \g{table} with a
pointer constant pointing to them, and then the table name 
and the string pointer are used:
\begin{program}
...
\end{program}
\noindent
Actually, as an actual affix a string translates into two affixes: first, a 
special table, and second, a pointer which points to the string in that table. 
The specified string is automatically added
to the special table. The compiler might (or might not) merge identical strings.
...
once, there is no need to designate a separate rule for this purpose.
An example is:
\begin{program}
...
The \g{selector list
pack} of a list cannot be the empty \pp|()|
either in a formal definition or in a declaration
(while the \A{} Manual allows this in formal list
definitions). The \g{selector list pack}, however, can be missing completely.
...
compilation. Originally \g{expression}s could be used at several places; this
...
the list block size (caliber) can be used.  List size estimates and repeat
...
constant tag; no expressions are allowed.
...
compiler reserves at least this much virtual space for the stack. (The final
...
\g{stack}s, the remaining space is distributed proportionally to the requested
relative amount.
...
list
in addition to selectors and the block size.
This \g{filling} is a sequence of integer denotations, constant tags
...
\g{filling} contains, in left to right order, the values which should be
...
If a \g{stack} has no declaration, namely, it is a list head with a size estimate,
...
\g{prototype} automatically gets
...
used for checking purposes only and do not imply any specific behavior.

The \pp|module| pragmat, if not skipped (i.e., if \pp|compile=on| is in
effect), defines the module name, and also sets \pp|prototype=public|.
...
\pp|else| pragmats is processed if \pp|TAG| is (or is not) in effect;
otherwise, it is skipped; and just the opposite for the text between
\pp|else| and \pp|endif|. The \pp|else| part may be missing.
The \pp|TAG| in the \pp|ifdef| (\pp|ifndef|)
pragmats can be any identifier (tag), and the compiler checks if this identifier 
has (has not) been defined until this point.
...
An \A{} module starts with a \pp|module| pragmat which defines the
name (title) of the module. After this, the module header follows, which contains the prototype 
of all exported (public) tags. The
body of the module, which defines the exported tags together
...
root can make any necessary initialization. If no initialization 
is necessary, the module can use an empty \g{root} as in the 
...
modules would wait for each other, so use this rule with care.
...
way private tags
can be created which are not available outside a library. Dictionary
...
novelty of this \A{} compiler. Allowing a variable number of affixes raises several
problems, and poses a main obstacle to keeping the tractability of the rule
properties. The approach used here meets two main requirements: on the one hand it
...
a pending, unseen block. If there is none, \pp|shift affix block| fails,
and the rule \pp|sum| ends. Otherwise, the execution continues, and a
...
out blocks are lost to this rule, and there is no way to reach or modify their
elements.
...
\end{program}
\noindent
put 7 in \pp|x|, 6 in \pp|y|, and $-7$ in \pp|z|.
...
it has two affixes: the table and the string pointer. Each format character
starting with \pp|\%| requires 
...
\end{program}
\noindent
Another application can be pushing and popping an unspecified number of elements
to a stack:
\begin{program}
'action'push+*>x:
...
replacement can result in a syntactically incorrect text, or in completely
...
Using the same \pp|macro| rule as above, the result has incorrect syntax:

\HH \pp|macro+u+\#| \HE becomes \pp|(u->\#->u, u->\#)|

\item While a macro can have a variable number of affixes, neither \pp|shift
affix block|, nor \pp|get affix blockno| is allowed. Rule \pp|is zero|
below checks if it has an argument with zero value; rule \pp|math| computes
the product of its arguments if none is zero, otherwise it computes their
sum:
\begin{program}
...
If the rule \pp|macro| is defined as
\begin{program}
'function'macro+>a+*+>q: q->b, call+a+b+*.
\end{program}
\noindent
where \pp|b| is some global variable, then after substitution the repeat
block can completely vanish, so calls of \pp|shift affix block| and \pp|get
affix blockno| would give syntax errors:
...
without it being passed to the macro explicitly:
...
formal out affix in the macro is turned into a local variable (which may be
removed during optimization). For 4) the macro substitution mechanism remembers the
last substituted formal affix, which gives the correct standard selector.
For 5) and other side effects, no warning is, or can be, given, but it
changes the semantics. So use macros with care.
...
\end{program}
\noindent
is transformed into the rule call
\begin{program}
@make+a+b[c]+c
\end{program}
\noindent
(recall that the character \pp|@| is a letter in library mode, see Section 
\ref{subsec:library}), and \pp|@make| is declared in the standard library as
an
\begin{program}
...
\emph{question}s with two input affixes, also defined in the standard
...
all equality tests use this rule. You might consider declaring \pp|@equal|
to be a \pp|macro|, which could also be done in library mode.

\section{Implementation details}

The target code of this \A{} compiler is {\sf C}. It is assumed that the
basic \A{} data type, which translates to \pp|int| in {\sf C}, is 32 bits long,
...
target machine to generate list fillings. Also, in datafile declarations the
...
\smallskip
\noindent
The \A{} list element \pp|L[i]| is translated into \pp|to\_LIST(L)->offset[i]|
...
in the program, thus whole lists can be moved freely in the memory, as long
as the
pointers \pp|offset| and \pp|p| are adjusted.
...
can be retrieved for both input and output datafiles, but one can only set the
position for an opened input datafile. No check is made to make sure that
the position is valid (so it can be set after the \pp|eof| indicator).
...
Appending to an existing \A{} datafile is not done as it raises problems.
The steps are: read the first block, check if it has the same metainformation as we have
...
first block, and compare the zones
...
local structure with the differences between the stored and requested pointer
values. When the next input is requested, it is checked whether it is a
pointer or not. If it is numerical, pass it. If it is a pointer, check which list
...
encoding as {\sf C}
strings whose last byte is \pp|{\char92}0|.  If the string is in list \pp|L|
...
\HH\pp|c5 c6 c7 c8|\HE   second word, bytes \pp|c5|--\pp|c8|
...
Each rule declaration is translated into a {\sf C} procedure declaration. If
...
affix block. Affixes which are neither \g{out} nor \g{inout} ones
...
string specified in the \g{external declaration}.
...
responsible for initializing this structure. The routine is called with two
...
external declaration is used as the name of a {\sf C} procedure which is
responsible for
initializing the structure. The procedure is called with a single argument:
...
mechanism as the compiled rules, see Section \ref{subsec:c-rules}. The
compiler even generates prototypes with the corresponding parameter
settings; the {\sf C} routine implementing the external rule can be compiled
and linked independently. Several standard library rules are implemented
this way.

If the first character in the \g{string denotation} of the
external rule is not a
letter, then another calling mechanism is used: all affixes, independently
...
translates to \pp|\_\_incr(ptr)|. The {\sf C} header file contains the
...
The \g{dummy affix} \pp|\#| translates to nothing, thus it leaves an empty parameter
...
discarded, then this external rule is not called at all. The same
optimization is not applied to non-external rules (only a warning is given),
as such a rule can check whether some condition holds and abort the program
depending on the outcome.

If the \g{string denotation} of the external rule starts with a \pp|@|, then
this character is replaced by an underscore. For each out argument,
depending on whether it is the \g{dummy symbol} or not, a \pp|0| or a \pp|1|
character is appended to the rule string. Finally, \g{dummy symbol}s are
discarded from the argument list. In the standard library the external rule
\pp|divrem| has two out affixes, and its \g{string denotation} is
\pp|@divrem|. Accordingly, four {\sf C} calls could be generated:
...
quotient or remainder is discarded; and the two parameter \pp|\_divrem00|
when no result is requested at all.
