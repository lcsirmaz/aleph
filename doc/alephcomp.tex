...
\parindent=0pt\color{green!30!black}\tt
...
In general, resources provided by a plain module \pp|A| are not
automatically avaiable for other modules, only if \pp|A| is required
(possibly through a request chain) explicitly. In contrast, designating
\pp|A| as a user library, any module can use resources provided by
\pp|A| without requesting it. A user library can request other modules;
these requests determine another visibility structure which is independent
of, and is above, the visibility structure of the plain modules. Resources
provided by the collection of user libraries are those provided by all
modules in the library visibility structure.
...
transparently if it chooses so) all the primitives defined in the standard
library without any further arrangements.
...
\HH\pp|x!=y|\HE  for \pp|x| and \pp|y| differ;
...
%
This extension comes handy when the content of a newly added block on the
top of a stack is to be manipulated. The drawback is that it prevents the
compiler reporting a parameter type mismatch when a list is used for a plain
value by mistake. This extension causes, quite unexpectedly, extra
...
A \g{string denotation} can be used as an \g{actual affix}. This extension
...
\end{program}
\noindent
The string denotation translates into two affixes: first, a special internal
table (which is not available in any other way), and second, a pointer which
points to the string in that table.
...
\pp|_rule_| in a macro text reflect the source line of the macro definition
...
reasonable amount of look-ahead. Nevertheless, the compiler issues a warning
when it finds an old style extension and ecourages the alternate syntax.
...
main novelties of this implementation. Variable number of affixes raises
...
A \g{formal affix sequence}, which defines the number and types of affixes
the \g{rule} accepts, may
...
this order. The rule body starts by setting \pp|a| to zero. The two \pp|add|
...
none, the rule \pp|shift affix block| fails, and \pp|sum| ends indicated by
the next \g{alternative} \pp|+|. Otherwise a
...
(pending) affix blocks of the actual calling rule are passed as arguments.
The rule \pp|negate| defined as
\begin{program}
...
to add up the value of the rest affixes, and then subtract the result from
the sum of the first two. Thus the calls
\begin{program}
...
pending affix blocks in \pp|n|. This number is always positive and it is
...
In the \A{} compiler this feature has been used mainly to format error messages,
...
correctly only when the actual stack \pp|st| has calibre (block size)
exactly one, as \pp|unstack+st| discards a complete block and not a single
element from the stack, see Section \ref{subsec:list-block}. The
...
\end{program}
\noindent
The \g{area} written between square brackets determines whether the alternative
...
were swapped); and the compiler gives a warning if it could happen that none
of the alternatives are chosen. When running the program and none of the
alternatives succeeds then the program aborts with an error message. The
last ``otherwise'' case can omit the \g{area} \pp|[:]| matching every possible
value.
...
restricts them to \g{constant} and \g{variable declaration}s only.
...
In an \g{expression} integer denotations (both decimal and hexadecimal),
...
\ref{section:module}), modular roots can perform all necessary initialization 
for the module. Modules which do not require initialization should use an
empty \g{root}: 
\begin{program}
'root'+.
\end{program}
...
rule requires the name of the module (see Section
...
the \pp|wait for| rule returns immediately. If there are several modules
with the same name, all those module roots are called.
...
\subsection{List declarers}

To distinguish \g{table declaration}s from prototypes and fillings (see
Sections \ref{subsec:prototype} and \ref{subsec:filling}, respectively), the
syntax of \g{table} and \g{stack declaration}s is changed to resemble that
of the \g{formal affix}es. In a \g{table declaration} the declared \g{tag}
is followed by \pp|[]|. In a \g{stack declaration}, in addition, the
optional \g{field list pack} is preceded by the \g{size estimate} enclosed
between \pp|[| and \pp|]|, as in
\begin{program}
...
The \g{size estimate} in a \g{stack declaration} specifies how much virtual
address space this stack requires. The estimate can be \emph{fixed},
\emph{relative}, or \emph{empty}. In the first two cases the
limit must be either an integer denotation or a constant tag; no expressions
are allowed.
\begin{itemize}
...
\item[\textbullet] If the size estimate is left empty, the stack size (both
...
size is 1 and the selector of that element is the name of the list -- this
is called the
\emph{standard selector}. The standard selector is used
implicitly when the list is indexed without specifying any selector. If the
...
separated by an equal sign as happens in the \g{stack declaration}
\begin{program}
'stack'[1](s1,s2=t1,s3=t2)stack[].
\end{program}
\noindent
To emphasize which selectors are used together, multiple \g{selector pack}s
are accepted. Each pack must have the same number of selectors, the
\g{dummy symbol} \pp|\#| can be used as a placeholder. Thus the previous
\g{declaration} can also be written as
\begin{program}
'stack'[1](s1,s2,s3)(\#,t1,t2)stack[].
\end{program} 
\noindent
If a list is defined with selectors, then a matching prototype must also 
...
The block sizes of the formal and the corresponding actual lists are
compared as follows.
\begin{itemize}
...
and blocks; no expressions are allowed. Each item, except for strings, can be
...
to \pp|T|, is the same as \pp|>>T| at the start of the program run).
...
(possibly a pointer constant), but not a string. In the block this
...
'exit'error+>x:
   x>=0,exit+0;
   fprintf+STDERR+"Exit level \%d\%n"+x,exit+1.
\end{program}
...
for appending (appending is for character files only); the last two arguments
...
the path string. No file can be opened for reading and writing
simultaneously.
...
character files \pp|STDIN| and \pp|STDOUT| declared in the standard \A{}
library correspond to the first two streams. The standard streams can be used 
without opening.
...
and generated in the current directory. The \pp|-m| option marks that the
following argument should be processed as if it were required; the \pp|-y|
option marks the following argument is to be processed as a user library.
The \pp|-I| option specifies the list of search directories for included
files, requested modules and user library modules. Finally the \pp|-L|
...
Before touching an indexed element, the value of the index is checked to be
...
rules and the number of calls are printed to \pp|stderr|. The list
...
followed by the values of its incoming \g{in} and \g{inout} affixes. As this would
...
command-line argument. (This switch is swallowed and not passed to the \A{}
program.)
Without this switch only the lastly executed 30
...
Frequently the \emph{call stack}, the hierarchy of pending rule calls
gives more information. When adding the \pp|-g| switch to the linker it
produces an \A{} binary which keeps track of this call stack, and prints it
...
the compiled program, but requires no change in the source. The library routine
\begin{program}
'action'backtrack.
\end{program}
\noindent
prints out the actual call stack to \pp|stderr| starting with the
rule containing this rule and ending with the program root. In the case the program is \emph{not} linked
...
In general, a module is an incomplete program part which can be compiled independently,
and which provides resources to be used by the other parts of the
program. Modules are frequently written so that it can be reused by several
programs. In an \A{} module the first non-comment line is usually a
\g{pragmat}
...
\noindent
The module name \pp|XXX| must be a valid \A{} identifier; it is written
without quotation marks. According to the best practice the name of the
file containing the module source and the module name should resemble each other.
It is particularly important as \A{} modules are invoked by
specifying source file names, and not module names. While not recommended,
different modules (that is, modules in different source files) can share
the same module name, and, consequently, the same namespace.

In the module the initial \pp|module| pragmat is followed by the public
part, called the \emph{head} of the module. The head contains the prototypes of the
...
\noindent
When the module is compiled, the source file is read with the initial
implicit instruction \pp|'pragmat'compile=on.| The \pp|module=sample|
pragmat in the first line defines both the name and the namespace of this
module to be \pp|sample|, and it also sets the \pp|prototype| pragmat to
\pp|public|. Next, the module head is parsed where the compiler marks all
prototyped tags to be exported (as instructed so by the current setting of
\pp|prototype|, see Section \ref{subsec:prototype}), which also means that
the prototyped tags must have a definition somewhere in the body of the
module. The condition in the \pp|if=compile| pragmat holds, thus the
material in the module body is processed: the \g{stack} and \g{rule}
\g{declaration}s are parsed and executed. The empty root rule (Section
...
against the prototypes and are prepared for export using the module
namespace.
...
\end{program}
\noindent
specifying the file name (between quotation marks) of the module source.
...
again the name and the namespace for the duration of processing \pp|fsample|, 
but sets the
...
compiled (the file name is specified in the command line), an implicit
...
and sets \pp|prototype=public|. If \pp|compile=off|, then it 
sets \pp|prototype=import| as it reads a module head.
...
algorithm without defining the rule which compares the elements to be sorted.
The skeleton of this module can be
...
\pp|qless| must be declared with an explicit qualifier:
...
and it is the linker's responsibility to replace it by the rule declared in
...
\pp|@equal| to be a macro, Section \ref{sec:macro}.
...
or in the main text; see an example in Section \ref{subsec:redef-library}.
...
\subsection{Stacks}\label{subsec:stdlib-stack}
...
with or without an explicit selector block.
%
...
the memory allocated to the stack \pp|st|; physical memory will be reallocated again
when \pp|st| is expanded. This rule replaces \pp|delete+st| from the Manual
using the more standard terminology. Additional memory can be requested in
bulk by calling \pp|request space+st+n|. After a successful return the
actual bounds of the stact \pp|st| do not change, but there are
...
The \pp|wait for+"xxx"| rule can be used to synchronize module initializations.
...
If not, calls those roots and waits until they return. For details see Section
\ref{subsec:root-rule}.

\smallskip

If the \A{} program is linked with the \pp|-g| switch, the
rule \pp|backtrack| prints out the complete call stack starting with the
rule containing this call and ending with the program root.
Consult Section \ref{subsec:debug} from more details.
\ignore
...
computes the hash of a block of words starting at the list element pointed
by \pp|p| and ending at \pp|>>t| (the last element of the list \pp|t|). This hash serves only as a tool to
...
An \A{} \g{transput} has strict semantics: it is evaluated from left
to right using the newly assigned values when necessary. The \g{transput}
\begin{program}
>>ST -> n -> ST[n]
\end{program}

\noindent
sets first \pp|n| to the address of the top of the stack \pp|ST|, and then
stores this value at the top element of \pp|ST|, using the newly assigned
value of \pp|n|. The semantics of the corresponding {\sf C} construct
\pp|ST->offs[n]=(n=ST->upb)| is \emph{undefined} as the {\sf C} compiler is
free to choose the order in which the two sides of the assigment are
evaluated. It can compute first the address of \pp|ST[n]|, and then execute
the assignment to \pp|n|, or the other way around. To avoid the ambiguity,
the \A{} compiler generates successive {\sf C} assignments, storing the
source in a temporary place when any of the destinations is indexed. The
genereted {\sf C} code for the above example is

\begin{program}
{\char123}int tmp=ST->upb; n=tmp; ST->offs[n]=tmp;\char125
\end{program}
