...
address space this stack requires. The estimate is given between square
brackets, and can be \emph{fixed}, \emph{relative}, or \emph{empty}. (This
last one is not allowed in the original \A{}.) In the first two cases the
...
the character sequence \pp|\char37 n| is replaced by a single newline. The
pragmat \pp|std
...
\noindent
When the module is compiled, the source file is read with the initial implicit
